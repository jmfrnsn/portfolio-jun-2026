...
`ORNAMENT_ADMIN_SECRET` protects website archive/restore actions on source detail pages (`POST /api/ornaments/sources/:id/archive` and `/unarchive`). The UI prompts for it once per browser session.
