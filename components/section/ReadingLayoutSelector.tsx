...
import { READING_LAYOUTS } from "@/lib/reading-layout";

export function ReadingLayoutSelector() {
  const { layout, setLayout } = useReadingLayout();
  const [open, setOpen] = useState(false);
  const rootRef = useRef<HTMLDivElement>(null);

  const activeLabel =
    READING_LAYOUTS.find((option) => option.id === layout)?.label ?? layout;
